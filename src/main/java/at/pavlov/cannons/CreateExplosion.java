...
//    }
    ...
    public void detonate(FlyingProjectile cannonball, org.bukkit.entity.Projectile projectile_entity) {
        ...
        Location impactLoc = projectile_entity.getLocation().clone();
...
//        }
        ...
        // fire impact event
        ProjectileImpactEvent impactEvent = new ProjectileImpactEvent(projectile_entity, projectile, explosion_power, impactLoc,
                cannonball.getShooterUID());
        Bukkit.getServer().getPluginManager().callEvent(impactEvent);
        cannonBallHit(impactEvent);
        ...
        if (impactEvent.isCancelled()) {
            ...
            if (explosion_power >= 0) {
                ...
                Explosion explosion = new Explosion(((CraftWorld) Objects.requireNonNull(world)).getHandle(),
                        null, null,  new CustomExplosionDamageCalculator(), impactLoc.getX(), impactLoc.getY(), impactLoc.getZ(),
                        explosion_power, projectile.hasProperty(ProjectileProperties.INCENDIARY),
                        projectile.getExplosionDamage()?Explosion.BlockInteraction.DESTROY:Explosion.BlockInteraction.KEEP
        ...
        if (projectile.isImpactMessage() && cannonball.getProjectileCause() == ProjectileCause.PlayerFired) {
            this.plugin.sendImpactMessage(player, impactLoc, false);
        }
